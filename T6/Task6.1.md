...
Поддерживает только записи типа A и AAAA.
Если запись существует, то сервер моментально отправляет ответ.
Если соответствующей записи не существует, то сервер делает запрос
к другому серверу. 
TTL при ответе задает, но сам игнорирует.
